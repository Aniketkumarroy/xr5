...
void hexDigitToBinary(char hexDigit, char *binary, void (*error)(char*) = nullptr) {
    switch (hexDigit) {
        ...
        default: if (error != nullptr) error("Wrong hex digit provided\n");
    }
}
...
int32_t hexaDecimalToSignedDecimal(char *hex, int size){
    char binary[32], nhex[strlen(hex)];
    int32_t value = 0;
...
void ValueInterpreter(char *value, char binaryString[], unsigned short size, void (*error)(char*) = nullptr){
    ...
    if(strlen(value) > 2 && value[0] == '0' && value[1] == 'x'){
        ...
        }else if(error != nullptr) error("Invalid Hex value");
    }else{
        if (isdecimal(value)){
            decimalToBinary(atoi(value), binaryString, size);
        }else if(error != nullptr) error("Invalid Decimal value");
    }
}